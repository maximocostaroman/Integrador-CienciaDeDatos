...
def bue_flights_v3():
    ...
    def fetch_calendar_for_dest(dest: str, months: list[str], meta: dict) -> str:
        ...
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump({"destination": dest, "months": months_blob}, f, ensure_ascii=False)
        return out_file
        ...
        for path in dest_files:
            ...
            for mm, payload in months_map.items():
                ...
                for day, rec in data.items():
                    ...
                    hours_tot = round((duration or 0) / 60, 1) if duration is not None else None
                    is_extreme = (transfers is not None and transfers >= 3) or (hours_tot is not None and hours_tot > 40)

                    month = depart_dt.month
                    season = ("Summer" if month in (12,1,2)
                    ...
                    rows.append({
                        ...
                        "depart_date": depart_dt.to_date_string(),
                        "departure_at": rec.get("departure_at"),   # string con tz
                        "price_min_raw": price,
                        "transfers": transfers,
                        "duration_min": duration,
                        "hours_total": hours_tot,
                        "is_extreme": bool(is_extreme),
                        "dow": depart_dt.day_name(),
                        "month": month,
                        "season": season,
                        "days_to_departure": (depart_dt.date() - snap_local.date()).days,
                    })
